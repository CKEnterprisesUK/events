 ...
 * two publish prerequisites ({starts_at, ticket_types}); the venue and capacity
 * items are never blocking; and each blocking item's satisfied state agrees
 * with Event::publishBlockers() — the single source of truth the publish
 * controller enforces — so presentation and enforcement never drift.
...
class EventReadinessBlockingTest extends PbtTestCase
{
     ...
     * blocking items is exactly {starts_at, ticket_types}, the venue and
     * capacity items are never blocking, and every blocking item agrees with
     * publishBlockers().
     *
    ...
    public function test_blocking_checklist_items_match_the_publish_source_of_truth(): void
    {
        ...
        $this->forAll(
            ...
            ->then(function (bool $hasStartDate, bool $hasTicketType): void {
                ...
                $expectedBlockingKeys = ['starts_at', 'ticket_types'];

                $this->assertSame(
                    $expectedBlockingKeys,
                    $blockingKeys,
                    'blocking checklist items must be exactly {starts_at, ticket_types}',
                );
